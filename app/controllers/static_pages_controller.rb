...
include ActionView::Helpers::NumberHelper
  def home
	...
	@lastFive = Game.where("played_at <= :start_date AND (away_score >= 0) AND (away_team_id = '8' OR home_team_id = '8')",{start_date: Time.now-8.hours}).last(5)
	@season = Game.where("played_at >= :start_date",{start_date: DateTime.new(2015,2,12)})
	@a = Array.new
	@season.each do |game|
	  if game.home_score
